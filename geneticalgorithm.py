...
def evoluir_populacao(population, dicionario_cidades, elitismo=True):
    nova_populacao = []
    num_replace = len(population) - int(elitismo) #20 ou 19 dependendo do Elitismo (0 ou 1)
...
def ag(dicionario_cidades, num_generations=100, population_size=20):
    population = pop_inicial(list(dicionario_cidades.keys()), population_size)
    for _ in range(num_generations):
        population = evoluir_populacao(population, dicionario_cidades, elitism=population_size // 2)
    best_individual = min(population, key=lambda x: aptidao_individuo(dicionario_cidades, [x])[0])
    best_cost = aptidao_individuo(dicionario_cidades, [best_individual])[0]
    return best_individual, best_cost

entrada = leitura_arquivo("wi29.tsp")
best_route, best_cost = ag(entrada)
end = time.process_time_ns()
tempo = (end - start)
print(f"O menor trajeto foi {best_route}, custando {best_cost}.")
